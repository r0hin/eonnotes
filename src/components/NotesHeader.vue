<template>
    <div id="notesHeader">
        ...
        <vs-dialog width="300px" not-center v-model="active3">
            <div class="cenetered">
                <h2>New Note</h2>
                <vs-input class="noteinput" v-model="noteinput" placeholder="Note Name"></vs-input>
            </div>
            <div class="btnCenter">
                <vs-button @click="active3=false; newNote()"> Ok </vs-button>
...
export default {
  ...
  methods: {
    async newNote() {
        ...
        const docRef = await db.collection('users').doc(user.uid).collection('notes').add({
            name: this.noteinput,
            status: true,
            content: '',
